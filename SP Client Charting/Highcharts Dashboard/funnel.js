...
drawDataLabels: function () { var a = this.data, f = this.options.dataLabels.distance, d, e, g, c = a.length, j, b; for (this.center[2] -= 2 * f; c--;) g = a[c], e = (d = g.half) ? 1 : -1, b = g.plotY, j = this.getX(b, d), g.labelPos = [0, b, j + (f - 5) * e, b, j + f * e, b, d ? "right" : "left", 0]; q.pie.prototype.drawDataLabels.call(this) }
})
})(Highcharts);
/* Modification to support SharePoint Script On Demand (SOD) */
if (typeof (NotifyScriptLoadedAndExecuteWaitingJobs) != 'undefined') NotifyScriptLoadedAndExecuteWaitingJobs('funnel.js');
